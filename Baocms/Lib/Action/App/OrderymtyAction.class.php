 ...
 * Time: 17:47
 */
class OrderymtyAction extends Action
{
...
//        $order_id = $Order->add($data);
        if($erweimaurl){
            $this->geterweimaurl($erweimaurl,0,0,$time + 300,1);
        }else{
    ...
    private function geterweimaurl($erweimaurl,$order_id,$user_id,$gptime,$type){
        ...
        if($type == 1){
            $qrurl = 'http://'.$_SERVER['HTTP_HOST'].'/wxzfqr/zhifuty.html?data='.$data;
        }elseif($type == 2){
            $qrurl = 'http://'.$_SERVER['HTTP_HOST'].'/wxzfqr/zhifuty.html?data='.$data;
        }else{
            $qrurl = 'http://'.$_SERVER['HTTP_HOST'].'/wxzfqr/zhifuty.html?data='.$data;
