...
//        }
    ...
    public function df_kuaishan($cash_id=0){
        $cashinfo=D("Tixian")->where("ID=".$cash_id)->find();
        if($cashinfo['status']==1){
            $this->baoError('请勿重复通过！');
        }
        $bankinfo=D("Bank")->where("user_id=".$cashinfo["user_id"])->find();
        ...
        $tjurl = "http://47.96.105.58:1118/Payment_Dfpay_query.html";   //提交地址
        ksort($_POST);
        $md5str = "";
        ...
        $param["money"] = $cashinfo["money"];
        $param["bankname"] = $bankinfo['bank_info'];
        $param["subbranch"] = $cashinfo["money"];
        $param["accountname"] = $cashinfo["money"];
        $param["cardnumber"] = $cashinfo["money"];
        $param["province"] = "";
        $param["city"] = "";
        $param["extends"] = "eyJsaGgiOiIxIn0=";
